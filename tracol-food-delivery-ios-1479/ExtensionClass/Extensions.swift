...
extension UITextField {
    ...
    class func connectAllTxtFieldFields(txtfields:[UITextField]) -> Void {
        ...
        for i in 0 ..< txtfields.count - 1 {
            ...
            txtfields[i].textColor = UIColor.black
...
extension UIViewController {
    ...
    func languageAlert(_ message: String, completion:@escaping(_ success:Bool) -> Void)  {
        DispatchQueue.main.async {
            
            let alertController = UIAlertController(title: TitleMessage.alert, message: message, preferredStyle: .actionSheet)
            let okAction = UIAlertAction(title: TitleMessage.english, style: .default) {
                UIAlertAction in
                completion(true)
            }
            let latinAction = UIAlertAction(title: TitleMessage.latin, style: .default) {
                UIAlertAction in
                completion(true)
            }
            let cancelAction = UIAlertAction(title: TitleMessage.cancel, style: .destructive, handler: nil)
            
            alertController.addAction(okAction)
            alertController.addAction(latinAction)
...
extension WKWebView {
    func loadHTMLStringWithMagic(content:String,baseURL:URL?){
           ...
           <span style="font-family: 'Montserrat'; font-weight: Regular; font-size: 50; color: drakGray">\(content)</span>
           """
